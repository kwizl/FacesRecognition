...
namespace FacesApi.Controllers
{
    ...
    public class FacesController : ControllerBase
    {
        ...
        {
            // MemoryStream deals with data in files. Used when getting bytes from another place
            using (var ms = new MemoryStream(2048))
            {
                // Reads from one stream to another
        ...
        private List<byte[]> GetFaces(byte[] image)
        {
            ...
            // Convert byte array into jpeg and Save image coming from the source
